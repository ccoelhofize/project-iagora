...
- Eighteen pre-stable executable schemas now include bounded acquisition plans, attempts, artifact versions, source-change reports, receipts, protected admission proposals, admission reviews, and temporary remote-package manifests alongside the civic-evidence and product contracts.
...
- Three exact bounded City API responses and acquisition events exist. One manually triggered GitHub Actions workflow can run the single reviewed six-school plan with read-only repository access, create a 14-day package, and hand only safe metadata to a separate receipt job. Its first controlled remote run succeeded with an exact-byte `unchanged` result and a closed `no_admission_required` receipt; the deadline monitor remains unexercised. A second protected workflow can revalidate one exact pending candidate and, after human environment approval, reject it or create only a dedicated branch and draft admission PR. Its required external environment is not configured and no real admission has run. No production connector fleet, general immutable raw store, database, public API, deployment, or public service exists.
...
- a manual read-only GitHub acquisition workflow, 14-day manifest-validated package, metadata-only durable issue receipt, and metadata-only day-10 reminder and expiry monitor; the acquisition and receipt path has one successful controlled run while the pending-candidate monitor remains unexercised;
- a protected two-job remote admission workflow, deterministic admission-proposal contract, exact package revalidation, bounded Git-data adapter, and admit-or-reject receipt transition; the external environment and real operational exercise remain pending;

### Remaining outcomes

- exercise the pending-candidate reminder and expiry path without widening its one-plan scope;
...
- configure and verify the protected remote-admission environment, then exercise the path on an eligible candidate without widening its scope;

...
The pre-stable generalized [attempt](../../contracts/v1/acquisition-attempt.schema.json), [artifact-version](../../contracts/v1/source-artifact-version.schema.json), [safe-receipt](../../contracts/v1/acquisition-receipt.schema.json), [admission-review](../../contracts/v1/admission-review.schema.json), and [source-change-report](../../contracts/v1/source-change-report.schema.json) contracts are executable. Deterministic compatibility projections expose the three historical acquisitions through those boundaries without fabricating missing plan versions, connector-rule versions, package dates, or decision times. A local one-plan component now emits safe attempts, candidate artifact metadata, and non-interpretive change reports into an append-only quarantine directory outside the repository. It does not emit durable remote receipts or admission decisions.

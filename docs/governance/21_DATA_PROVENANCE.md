...
The pre-stable generalized [attempt](../../contracts/v1/acquisition-attempt.schema.json), [artifact-version](../../contracts/v1/source-artifact-version.schema.json), [safe-receipt](../../contracts/v1/acquisition-receipt.schema.json), [admission-review](../../contracts/v1/admission-review.schema.json), [source-change-report](../../contracts/v1/source-change-report.schema.json), and [remote-package](../../contracts/v1/remote-acquisition-package.schema.json) contracts are executable. Deterministic compatibility projections expose the three historical acquisitions through those boundaries without fabricating missing plan versions, connector-rule versions, package dates, or decision times. The portable one-plan component emits safe attempts, candidate artifact metadata, and non-interpretive change reports into an append-only local quarantine. The remote adapter can bind these to a temporary package and durable metadata-only issue receipt while keeping admission absent. It has not yet emitted an operational remote receipt or admission decision.

...
The local vertical slice emits campaign-artifact authentication, three bounded City acquisitions, deterministic school-data normalization, an AI-assisted mapping proposal, administrative- and procurement-evidence reviews, passport projection, and publication-gate lineage events. Increment 0 acquisition validation projects each exact historical event into generalized attempt, artifact-version, and safe-receipt records. The local Increment 1 core emits plan, source-profile, connector-rule, attempt, artifact, validation, and source-change-report references for deliberate acquisition or replay. It binds exact fingerprints and records field-level differences without interpreting their civic meaning. Tests bind the three retained raw City fingerprints to both their original events and compatibility projections, verify school raw-to-normalized equality, distinguish framework maxima from expenditure, protect procurement amounts from holder-grain duplication, and verify deterministic passport replay. The BOAMP contributions record their response fingerprints and governed non-retention decisions rather than replayable raw artifacts. No shared persistent lineage store, GitHub attempt emitter, general dependency traversal, restriction propagation, admission lineage, or production replay mechanism exists. Physical representation remains open.

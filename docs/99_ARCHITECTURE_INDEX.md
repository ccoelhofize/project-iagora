...
| [`rfc/RFC-0001-portable-governed-source-acquisition.md`](rfc/RFC-0001-portable-governed-source-acquisition.md) | Draft, Increment 0 partial | Portable local and manually triggered GitHub Actions acquisition, human admission, and deferred external storage; contracts and fixtures exist, connector and workflows do not |
...
| [`../src/iagora/`](../src/iagora/) | Local prototype | Deterministic validation, historical acquisition compatibility projection, transformation, passport, territory home, Education dashboard, and printable programme dossier projection; no network connector |
...
The repository now contains a bounded local prototype, fifteen executable contract schemas, an active source inventory, authenticated campaign-artifact metadata with a stable fragment identifier, one separately resolvable canonical programme assertion, an explicit review-pending candidate-correspondence proposal, a three-stage fail-closed review packet, a partial administrative-evidence chain, a partial candidate service and citywide works-framework procurement chain, three exact bounded City API responses with acquisition metadata, one approved but non-executing six-school acquisition plan, deterministic historical compatibility projections, one explicitly synthetic pending-admission fixture, validated product projections, an initial test suite, and a minimal CI workflow. The packet preserves all five manually invoked advisory AI runs across mapping versions and corrections. The two applicable runs for corrected mapping version 0.3.0 both recommend `accept` for continued local POC use; one interim maintainer decision is pending. Two separated independent human reviews remain required before public release. No AI output counts as human review. The static product projection contains a Clermont-Ferrand home, an Education dashboard, a policy-lineage timeline, and a detailed printable dossier governed by the accepted multidimensional method in ADR-0010. The home has an honest macro-series placeholder rather than a city-wide KPI, and the non-Education themes remain absent or programme-limited. The repository does not contain a completed maintainer mapping review, completed independent mapping reviews, a generalized review workflow, a real macro city visualization, generalized indicator explorer, production report service, executable source connector, network acquisition, quarantine store, GitHub acquisition or admission workflow, general raw-evidence store, parser sandbox, database, backend service, public API, approved public frontend, account system, AI service, search index, production security control set, telemetry stack, release, SaaS capability, or deployment.

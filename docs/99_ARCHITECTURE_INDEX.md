...
| [`rfc/RFC-0001-portable-governed-source-acquisition.md`](rfc/RFC-0001-portable-governed-source-acquisition.md) | Draft, Increments 0–3 implemented | Portable local and manually triggered GitHub Actions acquisition, protected draft-PR admission, and deferred external storage; first acquisition and receipt exercised, admission unexercised |
...
| [`development/ACQUISITION.md`](development/ACQUISITION.md) | Draft | Safe local replay, manual acquisition, quarantine, temporary package, and protected admission boundary |
...
| [`../contracts/v1/admission-proposal.schema.json`](../contracts/v1/admission-proposal.schema.json) | Executable prototype | Exact package, receipt, decision, candidate, and deterministic target binding between read-only validation and protected writes |
...
| [`../contracts/v1/remote-acquisition-package.schema.json`](../contracts/v1/remote-acquisition-package.schema.json) | Executable prototype | Fingerprinted temporary remote-package manifest, raw-byte eligibility, and fail-closed admission state |
...
| [`../src/iagora/`](../src/iagora/) | Bounded portable prototype | Deterministic validation and product projection plus one registered Opendatasoft connector, constrained HTTPS transport, replay, structural validation, append-only quarantine, temporary remote packaging, safe issue receipts, deterministic receipt transitions, exact admission proposals, and bounded branch/draft-PR admission adapters |
| [`../tests/`](../tests/) | Executable tests | Contract, evidence, scope, replay, publication, accessibility, SSRF, redirect, quarantine, deduplication, local-runner equivalence, package, receipt, expiry, admission-tampering, GitHub-boundary, and workflow guardrails; acquisition and admission tests use retained or injected responses |
...
| [`../.github/workflows/governed-acquisition.yml`](../.github/workflows/governed-acquisition.yml) | Implemented, exercised once | Manual one-plan acquisition, read-only checkout, 14-day package, and separately privileged metadata-only receipt issue; first result was exact-byte `unchanged` |
| [`../.github/workflows/acquisition-receipt-monitor.yml`](../.github/workflows/acquisition-receipt-monitor.yml) | Implemented, unexercised | Metadata-only day-10 reminder and fail-closed expiry transition; never contacts a civic source |
| [`../.github/workflows/governed-admission.yml`](../.github/workflows/governed-admission.yml) | Implemented, externally disabled and unexercised | Read-only exact candidate revalidation followed by protected admit-or-reject handling; admit creates only a dedicated branch and draft PR |

## Planned capabilities and absent artifacts

The repository now contains a bounded portable prototype, eighteen executable contract schemas, an active source inventory, authenticated campaign-artifact metadata with a stable fragment identifier, one separately resolvable canonical programme assertion, an explicit review-pending candidate-correspondence proposal, a three-stage fail-closed review packet, a partial administrative-evidence chain, a partial candidate service and citywide works-framework procurement chain, three exact bounded City API responses with acquisition metadata, one approved six-school acquisition plan, deterministic historical compatibility projections, one explicitly synthetic pending-admission fixture, a bounded Opendatasoft connector and constrained transport, append-only local quarantine, offline replay, deterministic change reporting, a manual read-only GitHub acquisition workflow, 14-day package manifests, metadata-only issue receipts, a metadata-only deadline monitor, an externally disabled protected admission workflow, validated product projections, an initial test suite, and minimal CI. The acquisition and receipt path has one controlled exact-byte `unchanged` run; the deadline monitor and admission path are implemented but unexercised, and their tests use retained or injected responses. The packet preserves all five manually invoked advisory AI runs across mapping versions and corrections. The two applicable runs for corrected mapping version 0.3.0 both recommend `accept` for continued local POC use; one interim maintainer decision is pending. Two separated independent human reviews remain required before public release. No AI output counts as human review. The static product projection contains a Clermont-Ferrand home, an Education dashboard, a policy-lineage timeline, and a detailed printable dossier governed by the accepted multidimensional method in ADR-0010. The home has an honest macro-series placeholder rather than a city-wide KPI, and the non-Education themes remain absent or programme-limited. The repository does not contain a completed maintainer mapping review, completed independent mapping reviews, a generalized review workflow, a real macro city visualization, generalized indicator explorer, production report service, configured or operationally exercised governed admission, civic-source scheduling, general raw-evidence store, parser sandbox, database, backend service, public API, approved public frontend, account system, AI service, search index, production security control set, telemetry stack, release, SaaS capability, or deployment.

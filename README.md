...
│   └── LICENSE.md
├── reviews/
│   └── REVIEW-0001-campaign-to-programme-mapping.md
...
The current implementation authenticates a short, unquantified schoolyard-regreening commitment from a 2019 archived campaign page, preserves three exact bounded City API responses, and validates their school and procurement projections. It now also contains the first pre-stable governed-acquisition contracts, an approved six-school acquisition plan, deterministic compatibility projections of the three historical acquisitions, and a bounded acquisition core. That core builds the registered Opendatasoft request, applies constrained HTTPS transport rules, validates the exact response scope, supports offline replay, deduplicates retained candidate bytes in an append-only quarantine store outside the repository, and produces a field-level source-change report. A manually triggered GitHub Actions adapter runs that same one-plan core with read-only repository access, creates a manifest-validated 14-day review package, and passes only safe metadata to a separate issue-receipt job. A metadata-only monitor records one day-10 reminder and fail-closed expiry. A separate Increment 3 workflow now revalidates one exact pending package in a read-only job and, only after protected human approval, can reject it or create a dedicated branch and draft admission PR. It cannot write directly to `main`, merge, canonicalize, publish, or contact the civic source. This path remains externally disabled and operationally unexercised until the `governed-admission` environment and enablement variable are configured. The [first controlled remote exercise](docs/development/ACQUISITION.md#first-controlled-remote-exercise) succeeded on 5 August 2026: the City response exactly matched the governed 29 July bytes, so the receipt closed as `no_admission_required` and no raw bytes were duplicated or admitted. The deadline monitor remains operationally unexercised because that run created no pending candidate. Civic scheduling and production operation remain absent. Tests use retained or injected responses and do not contact the civic endpoint or GitHub API. The school subset covers Nestor-Perret, Pierre-et-Marie-Curie, and Jean-Zay. Ten metadata-only municipal documents establish adopted policy, programme-level budget authorization and expenditure, reported site delivery, funding forecasts, and the documented 2015 and 2018 education-policy predecessors without conflating those stages. A separate procurement bundle documents candidate evidence for a 2020 study, a 2021 citywide schoolyard works framework, a separate 2022 works-framework competition, and 2025 design and user-assistance services while preventing holder-level amount duplication and treating framework maxima as non-spending values. The source records do not directly name the Respire programme or identify a purchase order or pilot school, so school-attributable works, payment, and competent completion remain unestablished. BOAMP raw bytes remain excluded pending rights review. An explicit AI-assisted proposal records a cautious `candidate_correspondence` between one essential campaign component and the later programme. The first advisory cycle produced one `accept` and one `request_changes`; corrected mapping version `0.3.0` gives the campaign fragment a stable identifier, resolves the target through a separate canonical assertion, and corrects overbroad adoption wording. The current methodology and evidence-authority audits both recommend `accept` for continued local POC use, so one human maintainer decision is now pending. This interim decision cannot authorize publication or count as independent review; two separated human reviews remain required before public release. The Knowledge Passport and a deterministic three-level static interface expose the same bounded records through a Clermont-Ferrand home, an education dashboard, and a detailed printable dossier. The home keeps the macro city chart as an explicit missing-data state and keeps unsupported themes as placeholders; it does not invent a generalized city KPI. No generalized indicator explorer, production report service, account system, public frontend, or SaaS deployment exists. Publication remains blocked because the mapping is not independently accepted, school-attributable works procurement and competent-completion records remain missing, administrative-document and BOAMP dataset rights remain pending, and no reviewed outcome or causal-impact evidence exists. The roadmap describes strategic milestones rather than guaranteed dates.
...
The [acquisition guide](docs/development/ACQUISITION.md) explains safe offline
replay, deliberate manual acquisition, quarantine, temporary remote packages,
and the protected but not yet operationally exercised admission boundary.
